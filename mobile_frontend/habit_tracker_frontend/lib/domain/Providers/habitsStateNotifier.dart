...
class HabitsStateNotifier extends StateNotifier<List<Habit>> {
  ...
  Future<void> loadNewHabits() async {
    final habits = await habitToList.getHabitsList();
    habits.fold(
      (objectError) {
        print('The Error is : ${objectError.errorMessage}');
      },
      (sucessObject) {
        state = sucessObject;
      },
...
final habitSampleProvider =
    StateNotifierProvider<HabitsStateNotifier, List<Habit>>(
      (ref) => HabitsStateNotifier(habitToList: ref.watch(listFeatureProvider)),
    );
