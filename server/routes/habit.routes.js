...
const api = makeInvoker((container) => {
  return { controller: container.resolve("habitController") };
});
...
router
  .route("/")
  .post(addHabitValidator, api("controller", "createHabit"))
  .get(api("controller", "getHabits"));
...
router
  .route("/:id")
  .delete(validateIdParam, api("controller", "deleteHabit"))
  .put(validateIdParam, updateHabitValidator, api("controller", "updateHabit"));
...
router.post(
  ...
  validateIdParam,
  markCompleteValidator,
  api("controller", "markAsCompleted")
);
